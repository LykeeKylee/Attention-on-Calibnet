...
def sparsify_cloud(S):
    ...
    with tf.device('/cpu:0'):
        ...
        point_limit = 4096
        no_points = tf.cast(tf.shape(S)[0], dtype=tf.int32)
        no_partitions = tf.cast(no_points/tf.constant(point_limit, dtype=tf.int32), dtype=tf.int32)
        # print(no_points)
        # print(no_partitions)
        # print(no_partitions * point_limit)
